...
| `<List items="{/rows}"><StandardListItem …/></List>` | `<List items="{/rows}"><StandardListItem … /></List>` | String-syntax aggregation binding with a row template works byte-identical to XML. Object form and `<For>` also work; see [cookbook.md §6](cookbook.md#6-bound-aggregation-with-a-row-template--four-spellings) and [structural sample](../packages/jsx-runtime-showcase/webapp/view/showcases/Structural.tsx). |
